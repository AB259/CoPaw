...
The chat page SHALL render a compact metadata row immediately above each visible non-user message turn. The row SHALL display the sender label selected from the current URL or initialized iframe context and the formatted message time in `MM-dd HH:mm`, and it SHALL align to the left edge of the non-user message block. When URL parameter `origin=Y` is present, or the page has retained the corresponding initialized iframe context after navigation, the sender label SHALL be `AI伙伴`; otherwise it SHALL be `小助 Claw`.
...
- **THEN** the metadata row shows the formatted time next to the selected non-user sender label
...
### Requirement: Non-user sender label follows URL origin

The chat page SHALL use the URL-sensitive sender label for non-user message headers.

#### Scenario: Rendering a default non-user message label

- **WHEN** the chat page renders a non-user runtime response card
- **THEN** the non-user metadata row displays `小助 Claw`

#### Scenario: Rendering an origin Y non-user message label

- **WHEN** the chat page renders a non-user runtime response card with URL parameter `origin=Y`
- **THEN** the non-user metadata row displays `AI伙伴`

#### Scenario: Rendering after origin Y navigation changed the URL

- **GIVEN** URL parameter `origin=Y` initialized the iframe context
- **WHEN** the chat page renders a non-user runtime response card after client-side navigation removes the URL parameter
- **THEN** the non-user metadata row still displays `AI伙伴`
