...
class InstanceStore:
    ...
    async def create_instance(
        ...
        if self._use_db:
            ...
            await self.db.execute(
                query, (instance_id, source_id, bbk_id, instance_name, instance_url, max_users)
            )
        return Instance(
    ...
    async def get_instance_with_usage(self, instance_id: str) -> Optional[InstanceWithUsage]:
        ...
        if self._use_db:
            ...
            if row:
                ...
                usage_percent = (current_users / max_users * 100) if max_users > 0 else 0
    ...
    def _rows_to_instances_with_usage(self, rows: list[dict]) -> list[InstanceWithUsage]:
    ...
    async def delete_instance(self, instance_id: str) -> bool:
        ...
        if self._use_db:
            ...
            query = "SELECT COUNT(*) as cnt FROM swe_instance_user WHERE instance_id = %s AND status = 'active'"
            row = await self.db.fetch_one(query, (instance_id,))
    ...
    async def update_allocation_instance(
        ...
        if self._use_db:
            ...
            """
            result = await self.db.execute(query, (new_instance_id, user_id, source_id))
            return result > 0
        return False
    ...
    async def get_overview_stats(self) -> dict:
        ...
        if self._use_db:
            ...
            critical_count = 0
            for r in rows:
                level = calculate_warning_level(r["current_users"] or 0, r["max_users"] or 100)
                if level == "warning":
                    warning_count += 1
                elif level == "critical":
                    critical_count += 1
