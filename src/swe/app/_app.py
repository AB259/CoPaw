...
async def _initialize_wealth_plan_store(
    app: FastAPI,
    db_connection: Any | None,
) -> None:
    """初始化财富工作台规划存储并幂等建表。"""
    try:
        ...
        await wealth_plan_store.initialize()
        app.state.wealth_plan_store = wealth_plan_store
        if wealth_plan_store.is_available:
            logger.info("Wealth plan storage initialized")
        else:
            logger.warning("Wealth plan storage uses memory fallback")
    except Exception as e:
        logger.warning("Failed to initialize wealth plan storage: %s", e)
...
async def lifespan(
    ...
    await _initialize_wealth_plan_store(app, db_connection)
