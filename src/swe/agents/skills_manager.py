...
def _extract_requirements(post: dict[str, Any]) -> SkillRequirements:
    ...
    elif "swe" in metadata:
        requires = metadata["swe"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"SWE_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    available as ``SWE_SKILL_CONFIG_<SKILL_NAME>`` within the same scope.
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="swe_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"swe_skill_stage_{skill_name}_"),
    )
