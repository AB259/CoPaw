...
def test_console_chat_stop_accepts_logical_session_id(
    monkeypatch,
) -> None:
    app = FastAPI()
    app.include_router(console_router.router)
    ...
    class _StopTaskTracker:
        def __init__(self) -> None:
            self.requested_run_keys: list[str] = []

        async def request_stop(self, run_key: str) -> bool:
            self.requested_run_keys.append(run_key)
            return run_key == "chat-existing"

    task_tracker = _StopTaskTracker()
    workspace = SimpleNamespace(
        channel_manager=_FakeChannelManager(),
        chat_manager=chat_manager,
    ...
    response = client.post(
        "/console/chat/stop?chat_id=session-existing",
    )
    ...
    assert task_tracker.requested_run_keys == ["chat-existing"]
