...
function LineBody(props: { children?: React.ReactNode }) {
  const { getPrefixCls } = useProviderContext();
  const prefixCls = getPrefixCls("operate-card");
  return (
    <>
      <div className={`${prefixCls}-line-body`}>{props.children}</div>
    </>
  );
}
...
function OperateCard(props: IOperateCardProps) {
  ...
  return (
    <>
      ...
      >
        <div
          className={classNames(`${prefixCls}-header`, props.header.className, {
        ...
        >
          <div className={`${prefixCls}-header-icon`}>{props.header.icon}</div>
          {typeof props.header.title === "string" ? (
        ...
        {props.body && open && (
          <div className={`${prefixCls}-body`}>{props.body.children}</div>
        )}
      </div>
    </>
  );
